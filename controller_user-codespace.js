...
(function () {
                ...
                <div class="control-section">
                    ...
                    <div style="display:flex; flex-direction:column; gap: 12px;">
                        ...
                        <div style="display:flex; gap:12px; align-items:center;">
                            ...
                            <input id="botCount" type="number" value="1" min="1" max="800">
    ...
    HTML.teamFindBtn.onclick = () => {
        ...
        const count = parseInt(HTML.teamFindCount.value) || 1;
        ...
        HTML.teamFindStatus.textContent = "Scanning...";
        HTML.teamFindBtn.disabled = true;
        HTML.stopTeamFindBtn.disabled = false;

        // Ask server to spawn scan bots (server will manage inactivity termination)
        packet("Y", hash, count, time, team);
    };
    ...
    let connections = [];   // { id, ws, url, statusEl, connectBtn, disconnectBtn, entryEl }
    let nextConnId = 0;
    ...
    function updateHeaderStatus() {
        ...
        const connected = connections.filter(c => c.ws && c.ws.readyState === WebSocket.OPEN).length;
    ...
    function saveCodespaces() {
        const urls = connections.map(c => c.urlInput ? c.urlInput.value.trim() : c.url).filter(Boolean);
    ...
    function createCodespaceEntry(url = "") {
        ...
        const conn = { id, ws: null, url, statusEl, urlInput, connectBtn, disconnectBtn, entryEl: entry };
    ...
    async function watcherCycle() {
        ...
        const down = connections.filter(c => !c.ws || c.ws.readyState !== WebSocket.OPEN);
        ...
        for (let remaining = WATCHER_WAIT_MS / 1000; remaining > 0; remaining--) {
            ...
            setWatcherText(`✓ ${lastCheck} — ${connected}/${connections.length} up — next in ${remaining}s`);
    ...
    HTML.connectNoob.addEventListener("click", () => {
        ...
        const count = Math.min(parseInt(HTML.botCount.value) || 1, 800); // hard cap at 800
        if (HTML.botCount.value > 800) HTML.botCount.value = 800;
        if (!hash) {
    ...
    setTimeout(() => {
        ...
        if (reconnectAllBtn) {
            reconnectAllBtn.onclick = (e) => {
                e.stopPropagation();
                connections.forEach(c => connectTo(c));
            };
        }
