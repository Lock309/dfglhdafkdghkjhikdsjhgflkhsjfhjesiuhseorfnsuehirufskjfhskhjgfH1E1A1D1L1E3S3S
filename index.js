(async () => {
  ...
  const arras = (function () {
    ...
    const run = function (x, config, oa) {
      ...
      async function onJoin() {
        ...
        // Notify parent (server) that this headless bot has joined the game
        try {
        ...
        lastOverride = target.overrideMode;

        if (target.sneakMode) {
          controller.press("KeyO");
        }
        lastSneak = target.sneakMode;
        ...
        if (config.type === 'scan') {
            setTimeout(() => {
                log("Scan period complete (2s), disconnecting.");
                devastate();
                process.exit();
            }, 2000);
        }
        ...
        if (a) {
          ...
          if (lastHash !== global.location.hash) {
            ...
            lastHash = global.location.hash
              if (config.type === 'scan' && global.location.hash && global.location.hash.length > 1) {
                try {
                  process.send({
                    type: 'scanFound',
                    report: {
          ...
          if (inGame && config.type === 'follow') {
            ...
            if (target.overrideMode !== lastOverride) {
              controller.press("KeyR");
              lastOverride = target.overrideMode;
            }
            if (target.sneakMode !== lastSneak) {
              controller.press("KeyO");
              lastSneak = target.sneakMode;
            }
    ...
    let arras = {
      ...
      create: function (o) {
        ...
        o.id = o.id !== undefined ? o.id : botId++;
        return run(script, o)
      }
    }
    if (options.start) {
  ...
  process.on('message', (message) => {
    if (message.type === 'start') {
      ...
      target.overrideMode = Boolean(message.overrideMode);
      target.sneakMode = Boolean(message.sneakMode);
